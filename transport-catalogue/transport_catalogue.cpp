...
void TransportCatalogue::AddStop(string_view name, double latitude, double longitude) {
	...
	//	if (longitude < -180.0 && 180.0 < longitude) { throw invalid_argument("долгота не верна"s); }
		//cerr << setprecision(8) << latitude << ':' << longitude << endl;
...
void TransportCatalogue::AddBus(string_view number, const vector<string_view>& stops, bool ring) {
	vector<Stop*> tmp;
	for (auto& stop : stops) {
		//cout << stop << ":" << stopname_to_stop_.count(stop) << endl;
...
const TransportCatalogue::Bus* TransportCatalogue::FindBus(string_view number) const {
...
void TransportCatalogue::AddDistanceStop(string_view name, string_view name2, int distance) {
	...
	//cout << stop1 << ',' << stop2 << ':' << distance << endl;
}
...
TransportCatalogue::BusInfo TransportCatalogue::GetBusInfo(string_view number) {
...
const set<string_view> TransportCatalogue::GetBuses() const {
	set<string_view> buses;
...
//const set<string_view> TransportCatalogue::GetStops() const {
